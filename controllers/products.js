...
exports.getSingleProduct = asyncHandler(async(req, res, next) => {
    ...
    try {
        ...
        if (!product)
            return next(
                new ErrorResponse(
                    `Resourse not found with id of ${req.params.productId}`,
                    404
                )
            );
        product.views += 1;
        ...
        file.mv(
            ...
            async(err) => {
                ...
                await Products.findByIdAndUpdate(
                    req.params.productId, { photo: file.name }, {
                        new: true,
                        runValidators: true,
                    }
                );
...
exports.rateproduct = asyncHandler(async(req, res, next) => {
    ...
    try {
        ...
        if (!product)
            return next(
                new ErrorResponse(
                    `Resourse not found with id of ${req.params.id}`,
                    404
                )
            );
